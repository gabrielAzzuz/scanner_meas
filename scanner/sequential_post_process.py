...
class ScannerPostProcess():
    ...
    def var_reconstruction_target(self, Lxy_Var = True, zr_Var = True,
        ...
        if Lxy_Var != False:
            field_varLxy = self.field
            Zs_var = []; Alpha_var = []
            for n in range(nxy_var):
                field_varLxy.zs(Lx = ((n+1)*Lxy_var+Lxy_var), Ly = ((n+1)*Lxy_var+Lxy_var),
                                  n_x = round((210*Lxy_var)*(n+1)), n_y = round((210*Lxy_var)*(n+1)),
                                  zr=0.0)
                Zs_var.append(field_varLxy.Zs); Alpha_var.append(field_varLxy.alpha[0,:])
